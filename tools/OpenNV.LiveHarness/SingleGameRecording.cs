...
{
    internal static async Task Run(string channel, int processId, int seconds, string encoder, string output)
    {
        if (seconds is < 1 or > 60 || !Path.IsPathFullyQualified(output) || File.Exists(output) ||
            !Path.IsPathFullyQualified(encoder) || !File.Exists(encoder)) throw new ArgumentException("Recording needs 1-60 seconds, an encoder and a fresh absolute output path.");
        ...
        var audio = Task.Run(() => HarnessProcessAudio.Capture(processId, seconds * 1000, audioDirectory));
        Process? writer = null;
        ...
        try
        {
            ...
            for (var index = 0; index < seconds * 30; index++)
            {
                ...
                if (Stopwatch.GetTimestamp() / (double)Stopwatch.Frequency - frame.Nanoseconds / 1e9 > 2)
                    throw new IOException("The gameplay viewport stream is stale.");
                if (format is null)
                {
                    format = frame;
            ...
            using var mux = Start(encoder, ["-hide_banner", "-loglevel", "error", "-i", video, "-itsoffset", delay.ToString("R", CultureInfo.InvariantCulture),
                ...
                "-t", seconds.ToString(CultureInfo.InvariantCulture), "-movflags", "+faststart", output], false);
            ...
            File.WriteAllText(Path.ChangeExtension(output, ".json"), JsonSerializer.Serialize(new { processId, seconds, frames,
