...
internal static class NativeOwnedContactAudit
{
    ...
    {
        if (arguments.Length is not (5 or 9 or 10)) throw new ArgumentException("Owned contact needs installation, reference, three metre coordinates, optional quaternion and optional nearby geometry.");
        ...
        if (arguments.Length == 10)
        {
            foreach (var other in cell.References.Where(value => value.FormKey != key && (value.Flags & 0x800) == 0))
            {
                ...
                if (point.DistanceTo(body.Position) > 8 || cell.BaseObjects[other.Base].ModelPath is not { } model || !model.EndsWith(".nif", StringComparison.OrdinalIgnoreCase)) continue;
        ...
        try
        {
            ...
            foreach (var direction in new[] { Vector3.Right, Vector3.Left, Vector3.Forward, Vector3.Back })
            {
                ...
                GD.Print($"OPENNV_OWNED_CONTACT_ESCAPE direction={direction} from={before} to={body.GlobalPosition} distance={before.DistanceTo(body.GlobalPosition):R}");
