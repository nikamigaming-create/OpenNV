...
internal static class FalloutGameSettingFloats
{
    ...
    private static readonly ConditionalWeakTable<FalloutPluginStack, Dictionary<string, float>> Resolved = new();

    internal static float Read(FalloutPluginStack records, string name)
    {
        // A loaded winning stack is immutable. Retain its resolved declaration
        // instead of scanning/rereading every GMST on each actor physics tick.
    ...
    private static float ReadUncached(FalloutPluginStack records, string name)
    {
