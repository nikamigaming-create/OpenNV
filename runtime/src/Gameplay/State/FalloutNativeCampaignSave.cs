...
{
    internal const string ExpectedSchema = "opennv-native-fnv-campaign-save/v18";
        ...
        string expectedSaveCompatibilityId)
    {
        ...
        if (state.Schema != ExpectedSchema && state.References?.Any(reference => reference.PackageMotion?.Patrol is not null) == true)
            throw new InvalidDataException("Patrol progress requires the current campaign save schema.");
        if (state.Schema is ExpectedSchema or EncounterZoneSchema && state.EncounterZones is null)
        ...
        if ((state.Schema != ExpectedSchema && state.Schema != EncounterZoneSchema && state.Schema != ActorOverridesSchema && state.Schema != PackageMotionSchema && state.Schema != IngestiblesSchema && state.Schema != ViewPitchSchema && state.Schema != ReferenceStateSchema && state.Schema != QuestClockSchema && state.Schema != SkyLightingSchema && state.Schema != GlobalClockSchema && state.Schema != QuestScriptsSchema && state.Schema != FeetAnchoredSchema && state.Schema != CapsuleCenteredSchema) ||
            (state.Scripts is null) != (state.Quests is null) ||
            (state.Globals is null) != (state.GameTime is null) ||
            (state.Schema is ExpectedSchema or EncounterZoneSchema or ActorOverridesSchema or PackageMotionSchema or IngestiblesSchema or ViewPitchSchema or ReferenceStateSchema or QuestClockSchema or SkyLightingSchema or GlobalClockSchema && state.Globals is null) ||
            (state.Schema is ExpectedSchema or EncounterZoneSchema or ActorOverridesSchema or PackageMotionSchema or IngestiblesSchema or ViewPitchSchema or ReferenceStateSchema or QuestClockSchema or SkyLightingSchema && state.SkyLighting is null) ||
            (state.Schema is ExpectedSchema or EncounterZoneSchema or ActorOverridesSchema or PackageMotionSchema or IngestiblesSchema or ViewPitchSchema or ReferenceStateSchema && state.References is null) ||
            (state.Schema is ExpectedSchema or EncounterZoneSchema or ActorOverridesSchema or PackageMotionSchema or IngestiblesSchema or ViewPitchSchema && state.PlayerViewPitchRadians is null) ||
            (state.Ingestibles is not null && (state.Schema is not (ExpectedSchema or EncounterZoneSchema or ActorOverridesSchema or PackageMotionSchema or IngestiblesSchema) || state.Vitals is null)) ||
