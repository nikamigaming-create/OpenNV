...
internal sealed class ReactiveReferenceBot
{
    ...
    private int _obstructionReplans;
    ...
    internal void Start(string reference, string mode, float distance)
    {
        ...
        _elapsed = _stalled = _waiting = 0; _replans = _obstructionReplans = 0; _error = null;
    }
    ...
    internal void Tick(float seconds)
    {
        ...
        try
        {
            ...
            if (move)
            {
                ...
                {
                    if (Vector3.Distance(state.Position, _segmentStart) < .1f)
                        throw new InvalidOperationException("Navigation segment ended without observed progress.");
                    // Only the next segment is capsule-verified. Replan from
                    // the observed arrival before entering the next corridor.
                    _path = null; _input(default, false); _steering.Reset();
                    _phase = "replanning-segment";
                    return;
                }
            ...
            if (_stalled > .7f)
            {
                ...
                if (++_obstructionReplans > 3)
                    throw new InvalidOperationException("Navigation obstructed after bounded replans: " + (state.Blocker ?? "no capsule progress"));
                _path = null; _stalled = 0; _steering.Reset(); _phase = "replanning-obstruction";
                return;
            }
            ...
            if (activate)
            {
                ...
                // The observed ray already hits the requested reference. A
                // further look adjustment here can move it off before Activate.
                _waiting = 0; _input(default, true);
            }
            else if (near && _mode is "approach" or "travel") Complete("arrival-observed");
