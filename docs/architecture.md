...
| `runtime/src/Campaigns/Fallout1/Fo1TacticalSession*.cs` | V13ENT player hex, BFS movement, selected target, bounded attack/rat turn, and HP/AP HUD, with the atomic save/restore codec isolated in its persistence module | Camera transforms, MAP parsing, or full AI formulas |
...
| `runtime/src/Campaigns/Fallout1/Fo1OwnedCaveKit*.cs` | Exact-source-topology continuous floor construction and verified owned cave-kit instantiation, with molded-wall geometry isolated from manifest composition | Fallout 1 topology derivation or camera policy |
...
| `runtime/src/Campaigns/Fallout2/Temple/Fo2TemplePresentationContract.cs` | Full cache/source/profile/recipe, floor-unprojection, and PNG identity validation for admitted Map 126/Map 3 presentation | DAT2 parsing, invented placements, gameplay, or runtime readiness |
| `runtime/src/Campaigns/Fallout2/Temple/Fo2MapSceneBuilder.cs` | Opaque owned-floor composition over exact MAP patch positions plus MAP-authored top-level object sprites; floor unprojection changes cache projection only | MAP/hex placement, collision authority, source selection, scripts, or parity |
