...
| Companions | Creature Follow and direct Dialogue packages now have movement/conversation owners; package position and clocks persist. Scaled creature health uses retained level selection. | Source package priority and native ED-E movement/stop/cold fixtures pass. Ordinary repair/recruitment, door transfer, perks/factions/combat-style effects and companion combat remain open. |
...
| Travel | Source doors, a moving exterior grid, LAND/LOD and retained reference state are connected. Source-portal A* uses native player-capsule clearance and bounded replanning. | The flat bot completed the Primm approach and entered Nash Residence. Both modes navigated ED-E's real Repair/Parts menus and exited; obsolete message prompts no longer discard the first choice. Repair/recruitment, following, complete routes and streaming spikes remain open. |
