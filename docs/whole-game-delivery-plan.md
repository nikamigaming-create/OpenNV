...
| Owned Fallout 2 character/Temple/Arroyo Caves route | `interactive bounded Hex launcher slice`; original owned picker plus distinct Narg/Mingan/Chitsa panels and one portrait/live curved-3D-panel toggle, premade Take plus source-backed Modify/Create, exact Map 3 exit serial 1738 into ARTEMPLE Map 126, and version-12 atomic appearance/gameplay cold restore. One retained branch uses exact tagged-Speech Cameron state, keeps Klint alive, reaches live ARVILLAG, applies input tile 11683 → 11482, and cold-restores. The alternative strict MAP/PRO/MSG-bound Villager branch exposes player AP movement, deterministic adjacent melee, defeat, exact nested Spear loot/equip, visible inventory, and cold restore in Temple. The branches do not merge or admit a dead-guardian village shortcut. Target AI/turns, general INT execution, broad combat/inventory, campaign-wide persistence, full campaign, FPS/OpenXR, and parity remain pending. | [Canonical FO2 branch ledger](evidence/fo2-first-slice-branch-ledger.md) |
| Owned New Vegas menu, intro, character creation, and Doc Mitchell opening | `interactive bounded evidence`; the latest native gate proves exact stage-8 furniture placement/independent chair-smoke clocks with no Doc/bed AABB overlap, owned cigarette plus explicitly adapted tip-anchored smoke, visible name entry, the 43-control default-male head-only preview, stage-40 exit-root handoff, and stage-55 autosave. It did not cold-resume or produce accepted media. Earlier stage-200 checkpoint/resume evidence is retained separately. Female/non-default live 3D faces, a full-body player actor, uninterrupted continuity, and visual parity remain pending | [Owned opening campaign-state contract](evidence/fnv-owned-opening-campaign-state-contract.md) |
| Doc Mitchell house/Goodsprings exterior/saloon ordered route | Current configured-input traversal/save and cold Continue pass against the admitted four-family cache; campaign save v6 owns source-derived vitals, CELL `00106185`, containers, and player transform. Only the authoritative current CELL renders, processes, collides, emits lights, and advances one-time actor grounding; linked CELLs remain hidden, frozen preload state. The exact six-actor package/condition source contract is audited, but its generic schedule/condition/quest executors are not implemented. Exact startup camera/player-root state, complete population/package AI, exterior surface/directional lighting, dynamic time/weather, player deposits, reverse traversal, neighboring exterior-grid streaming, and integrated OpenXR remain pending | [Current-CELL route](evidence/fnv-route-active-set-contract.md), [actor/package source contract](evidence/fnv-goodsprings-actor-package-contract.md) |
| Owned Fallout 3 menu, intro, CG00 character creation, and bounded birth room through persistent stage 100 | `interactive frontend`, not a freely playable presentation; exact stage-65/80/85/90 results, cue/fade/sound, seven of eight stage-100 commands, and cold restore work. A fresh profile compiles both movies and the CG01 stage-0/stage-5/Dad-speech-to-stage-10 contracts, but native stage-10 apply/restore is blocked by non-portable actor derivative paths in the previous presentation cache. The eighth stage-100 command, general package/dialogue/KF AI, wider Vault 101 world play, Hex, and VR remain pending | [Multi-game first slices](multi-game-first-slices.md) |
...
| JAM and TTW | TTW profile/effective-source registration and a bounded launcher-validated FO3 CG00→CG01-stage-5 contract work with isolated cache/save identities, but no TTW interpreter/world/save/xNVSE/JAM runtime exists. JAM is dependency- and portable-semantic-gated with bounded JVS sprint and JBT time-dilation semantics transported. Both routes remain disabled | [Mod policy](mods.md) |
