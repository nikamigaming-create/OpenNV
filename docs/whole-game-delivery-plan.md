...
| Owned Fallout 2 character/Temple/Arroyo Caves route | `interactive bounded Hex launcher slice`; premade Take plus source-backed Modify/Create for name/sex/age/exact SPECIAL, PRO-linked sex-correct AA idle/directional AB walking, exact Map 3 exit serial 1738 into ARTEMPLE Map 126, and version-4 atomic cold restore work. One strict MAP/PRO/MSG-bound Villager exposes player HP/AP, deterministic adjacent melee, defeat, exact nested Spear loot, visible inventory, and cold restore. The opaque non-source wall proxy is removed while owned wall FRMs/collision remain; target AI/turns, INT/dialogue, general combat/inventory, tag/trait editing, other animations, campaign-wide persistence, remaining exits, full campaign, FPS/OpenXR, and parity remain pending | [Multi-game first slices](multi-game-first-slices.md) |
