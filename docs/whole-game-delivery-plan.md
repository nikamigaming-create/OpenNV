...
| Owned Fallout 2 premade/Temple/Arroyo Caves route | `partial`; exact Map 126 and Map 3 graphs/caches, Temple topology and exit state, plus a visible owned Narg/Mingan/Chitsa selector with source stats/bios/portraits, sex-correct idle FRM, Take handoff to Map 3 tile 28707, physical floor support, bounded source-walk-gated movement, and atomic cold restore of the selected source state plus current Map 3 transform/mode; Modify/Create, editable fields, campaign-wide persistence, script/actor/combat/inventory systems, reciprocal runtime exits, full campaign, FPS/OpenXR, launcher promotion, and parity remain pending | [Multi-game first slices](multi-game-first-slices.md) |
