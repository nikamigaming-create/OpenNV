...
| Owned Fallout 2 character/Temple/Arroyo Caves route | `interactive bounded Hex launcher slice`; premade Take plus source-backed Modify/Create for name/sex/age/exact SPECIAL, PRO-linked sex-correct AA idle/directional AB walking, exact Map 3 exit serial 1738 into ARTEMPLE Map 126, and version-3 atomic cold restore work. The opaque non-source wall proxy is removed while owned wall FRMs/collision remain; tag/trait editing, other animations, campaign-wide persistence, scripts/actors/combat/inventory, remaining exits, full campaign, FPS/OpenXR, and parity remain pending | [Multi-game first slices](multi-game-first-slices.md) |
