...
class Fo1DestinationMedicLookTest(unittest.TestCase):
    def test_compiler_requires_nearest_medic_map_actor_script_message_and_open_door(self) -> None:
        ...
        self.assertIn("decode_medic_heal_player", tool)
    ...
    def test_runtime_executes_decoded_dialogue_targets_and_persists_the_procedure(self) -> None:
        ...
        self.assertIn("medic.EffectDialogueTargets.Contains", session)
        self.assertIn("_playerHitPoints += healing.PlayerHealing", session)
        self.assertIn("_classicPlayerStatus.Apply(healing)", session)
        self.assertIn("ClassicPlayerStatusState.Restore", session)
        self.assertIn("state = _classicPlayerStatus.Save()", session)
        self.assertIn("healing.GameTimeAdvanceMinutes", session)
        self.assertIn("PlayerMaximumHitPoints: _playerProfile.HitPoints", session)
        self.assertIn("playerHitPoints = _playerHitPoints", session)
        self.assertIn('_playerHitPoints = root.GetProperty("playerHitPoints")', session)
