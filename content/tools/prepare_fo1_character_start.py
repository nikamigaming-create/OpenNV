...
def colors_from_palette(data: bytes) -> list[tuple[int, int, int, int]]:
    ...
    for index in range(256):
        ...
        if red <= 63 and green <= 63 and blue <= 63:
...
def color_table_rgb(data: bytes, color_table_index: int) -> tuple[int, int, int]:
    ...
    if red > 63 or green > 63 or blue > 63:
        raise ValueError(
...
def decode_aaf_font(
    ...
    glyph_data_offset = 2060
    if len(data) < glyph_data_offset or struct.unpack_from(">I", data, 0)[0] != 0x41414646:
    ...
    if maximum_height <= 0 or maximum_height > 128:
        raise ValueError(f"Fallout bitmap font maximum height is invalid: {maximum_height}")

    records: list[tuple[int, int, int]] = []
    for code_point in range(256):
        width, height, offset = struct.unpack_from(">hhI", data, 12 + code_point * 8)
        if width < 0 or height < 0 or width > 128 or height > maximum_height:
    ...
    atlas = Image.new("RGBA", (cell_width * 16, maximum_height * 16), (0, 0, 0, 0))
    for code_point, (width, height, offset) in enumerate(records):
        ...
        if any(value > 7 for value in source):
            raise ValueError(f"Fallout bitmap glyph {code_point} has an invalid intensity")
        left = (code_point % 16) * cell_width
        top = (code_point // 16) * maximum_height + maximum_height - height
        for y in range(height):
            for x in range(width):
                ...
                if intensity:
                    atlas.putpixel(
                        (left + x, top + y),
                        (*tint_rgb, round(intensity * 255 / 7)),
                    )
...
def normalize_ogg_serial(path: Path, serial: int) -> None:
    ...
    while cursor < len(data):
        if cursor + 27 > len(data) or data[cursor : cursor + 4] != b"OggS":
        ...
        struct.pack_into("<I", data, cursor + 22, checksum)
        cursor = page_end
...
def parse_premade_gcd(data: bytes) -> dict[str, object]:
    ...
    if not name.strip() or len(name) > 11 or any(ord(value) < 32 for value in name):
        raise ValueError(f"Fallout premade GCD name is invalid: {name!r}")

    special_values = list(values[1:8])
    if any(value < 1 or value > 10 for value in special_values) or sum(special_values) != 40:
        raise ValueError("Fallout premade GCD has invalid allocated SPECIAL values")
    age = values[34]
    sex_index = values[35]
    if age < 16 or age > 35 or sex_index not in (0, 1):
        raise ValueError("Fallout premade GCD has an invalid age or sex")

    tag_indices = list(values[100:103])
    if (
        values[103] != -1
    ...
    raw_traits = list(values[104:106])
...
def prepare(
    ...
    try:
        ...
        with frame_pack_path.open("wb") as stream:
            ...
            stream.write(struct.pack("<IIIII", movie_width, movie_height, 15, 1, len(frame_paths)))
        ...
        normalize_ogg_serial(audio_path, 0x4F4E5631)
        ...
        if not 100.0 < audio_duration_seconds < 130.0:
            raise ValueError(
                f"Fallout Overseer audio duration is invalid: {audio_duration_seconds}"
            )
        if frame_pack_path.stat().st_size < 1_000_000 or audio_path.stat().st_size < 100_000:
        ...
        manifest = {
            ...
            "opening": {
                ...
                "durationSeconds": len(frame_paths) / 15.0,
