...
def render(
    ...
    source_canvas = Image.new("RGBA", crop_size, (5, 8, 7, 255))
    ...
    for label, tile, color in (
        (f"FIRST-RUN SPAWN {entry_tile}", entry_tile, (45, 225, 255, 255)),
        (f"DOOR {door_tile}", door_tile, (255, 205, 55, 255)),
    ):
        screen = classic_hex_screen(tile)
        point = (screen[0] - crop_origin[0], screen[1] - crop_origin[1])
        draw.ellipse((point[0] - 9, point[1] - 9, point[0] + 9, point[1] + 9), outline=color, width=3)
        draw.text((point[0] + 12, point[1] - 10), label, fill=color)
    draw.rectangle((0, 0, 1279, 38), fill=(3, 7, 5, 225))
    draw.text(
        (16, 12),
    ...
    comparison = Image.new("RGBA", (2560, 770), (8, 10, 9, 255))
    comparison.alpha_composite(source_canvas, (0, 50))
    comparison.alpha_composite(godot, (1280, 50))
    compare_draw = ImageDraw.Draw(comparison)
    compare_draw.rectangle((0, 0, 1279, 49), fill=(15, 19, 16, 255))
    compare_draw.rectangle((1280, 0, 2559, 49), fill=(15, 19, 16, 255))
    compare_draw.text((18, 17), "SOURCE MAP/FRM RECONSTRUCTION", fill=(238, 195, 72, 255))
    compare_draw.text((1298, 17), "GODOT — CORRECTED REVERSED FLOOR-X", fill=(105, 235, 110, 255))
    compare_draw.line((1279, 0, 1279, 769), fill=(238, 195, 72, 255), width=2)
    ...
    report = {
        ...
        "deltas": [
            ...
            {
                "symptom": "player began near the far map-entry fallback instead of just outside the Vault",
                "owner": "new-game spawn-state selection",
                "previous": "MAP header fallback tile 20090 rotation 0",
